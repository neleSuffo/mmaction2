...
@DATASETS.register_module()
class CustomActivityNetDataset(BaseActionDataset):
    """ActivityNet dataset for temporal action localization. The dataset loads
    raw features and apply specified transforms to return a dict containing the
    frame tensors and other information. The ann_file is a json file with
    multiple objects, and each object has a key of the name of a video, and
    ...
    def load_data_list(self):
        ...
        data_list = []
        anno_database = mmengine.load(self.ann_file)
        for video_id, video_info in anno_database.items():
            filename = f"{self.data_prefix['video']}/{video_id}.MP4"
            for annotation in video_info['annotations']:
                ...
                data_list.append({
                    'filename': filename,
                    'gt_segments': [start_time, end_time],
                    'gt_label': label
                })
        return data_list
...
class LoadCustomAnnotations(BaseTransform):
    ...
    def _load_labels(self, results: dict) -> None:
        ...
        for instance in results['instances']:
            ...
            # Map string labels to integer labels using the label_map
            if label in self.label_map:
                gt_label.append(self.label_map[label])
            else:
                raise ValueError(f"Unknown label '{label}' found in the annotations.")
    ...
    def _load_segments(self, results: dict) -> None:
        ...
        gt_segment = []
        for instance in results['instances']:
            segment = instance['segment'] # Extract segment (start, end)
            gt_segment.append(segment)
        # Store the result as an ndarray in (N, 2) format, where N is the number of segments
        results['gt_segment'] = np.array(gt_segment, dtype=np.float32)
